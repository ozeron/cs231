...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        conv_vals, conv_cache = conv_relu_pool_forward(X, W1, b1, conv_param, pool_param)
        N, F, HH, WW = conv_vals.shape
        D = F*HH*WW
        conv = conv_vals.reshape((N, D))
        ...
        dx, dw2, db2 = affine_relu_backward(dscores, hidden_cache)
        dw2 += self.reg*(W2)
        
        dconv = dx.reshape((N, F, HH, WW))
